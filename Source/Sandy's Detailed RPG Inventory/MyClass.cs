	...
	public class Sandy_Detailed_RPG_GearTab : ITab_Pawn_Gear
	{
		...
		protected override void FillTab()
		{
			...
			if (!viewlist)
			{
				...
				else if (!this.SelPawnForGear.RaceProps.Humanlike)
				{
			...
			if (this.ShouldShowOverallArmor(this.SelPawnForGear) && !viewlist)
			{
			...
			else if (this.ShouldShowOverallArmor(this.SelPawnForGear) && viewlist)
			{
			...
			if (this.IsVisible && this.SelPawnForGear.RaceProps.Humanlike && !viewlist)
			{
				...
				Rect newRect7 = new Rect(402f, 338f, 72f, 72f);
				GUI.DrawTexture(newRect7, ContentFinder<Texture2D>.Get("UI/Widgets/DesButBG", true));
				Color color = new Color(1f, 1f, 1f, 1f);
				GUI.color = color;
				Rect newRect8 = new Rect(374f, 172f, 128f, 128f);
				this.DrawColonist(newRect8, this.SelPawnForGear);
			}
			if (this.ShouldShowEquipment(this.SelPawnForGear) && !viewlist)
			{
				foreach (ThingWithComps current in this.SelPawnForGear.equipment.AllEquipmentListForReading)
				{
					...
					{
						Rect rect6 = new Rect(newRect.x, rect.yMax - 20f, 20f, 20f);
			...
			if (this.ShouldShowApparel(this.SelPawnForGear) && !viewlist)
			{
				foreach (Apparel current2 in this.SelPawnForGear.apparel.WornApparel)
				{
					...
					if (current2.def.apparel.bodyPartGroups.Contains(Sandy_Gear_DefOf.Hands) && !current2.def.apparel.bodyPartGroups.Contains(BodyPartGroupDefOf.Torso)
					    && current2.def.apparel.layers.Contains(ApparelLayerDefOf.Shell) && !current2.def.apparel.bodyPartGroups.Contains(Sandy_Gear_DefOf.Shoulders))
					{
					...
					if (current2.def.apparel.bodyPartGroups.Contains(Sandy_Gear_DefOf.Feet) && current2.def.apparel.layers.Contains(ApparelLayerDefOf.Shell)
					    && !current2.def.apparel.bodyPartGroups.Contains(BodyPartGroupDefOf.Legs))
					{
			...
			else if (this.ShouldShowApparel(this.SelPawnForGear) && viewlist)
			{
