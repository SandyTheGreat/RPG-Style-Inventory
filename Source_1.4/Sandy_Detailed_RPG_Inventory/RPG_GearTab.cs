	...
	public class Sandy_Detailed_RPG_GearTab : ITab_Pawn_Gear
	{
		...
		public override bool IsVisible
		{
			get
			{
				...
				return selPawnForGear.ageTracker.CurLifeStage != LifeStageDefOf.HumanlikeBaby && (this.ShouldShowInventory(selPawnForGear) || this.ShouldShowApparel(selPawnForGear) || this.ShouldShowEquipment(selPawnForGear));
			}
		}
					...
					//foreach (Apparel current2 in this.SelPawnForGear.apparel.WornApparel)
				{
					...
					switch (current2.def.apparel)
					{
						...
						/*case ApparelProperties a when (a.bodyPartGroups.Contains(BodyPartGroupDefOf.Torso) && a.layers.Contains(ApparelLayerDefOf.Belt)):
							...
							this.DrawThingRow_RPG(newRectTb, current2, false);
							break;*/
		...
		private void DrawThingRow_RPG(Rect rect, Thing thing, bool inventory = false)
		{
			...
			Text.WordWrap = true;
			string text2 = thing.LabelNoParenthesisCap.AsTipTitle() + GenLabel.LabelExtras(thing, 1, true, true) + "\n\n" + thing.DescriptionDetailed;
			string text3 = text + "\n" + text2 + "\n" + smass;
