	...
	public class Sandy_Detailed_RPG_GearTab : ITab_Pawn_Gear
	{
		...
		public override bool IsVisible
		{
			get
			{
				...
				return selPawnForGear.ageTracker.CurLifeStage != LifeStageDefOf.HumanlikeBaby && (!ModsConfig.AnomalyActive || !selPawnForGear.RaceProps.IsAnomalyEntity)
                    && (!ModsConfig.OdysseyActive || !selPawnForGear.RaceProps.IsDrone)
                    && (this.ShouldShowInventory(selPawnForGear) || this.ShouldShowApparel(selPawnForGear) || this.ShouldShowEquipment(selPawnForGear));
			}
		}
		...
		private bool CanControl
		{
			...
			{
				Pawn selPawnForGear = this.SelPawnForGear;
				return !selPawnForGear.Downed && !selPawnForGear.InMentalState && selPawnForGear.CarriedBy == null && (selPawnForGear.Faction == Faction.OfPlayer || selPawnForGear.IsPrisonerOfColony) 
					&& (!selPawnForGear.IsPrisonerOfColony || !selPawnForGear.Spawned || selPawnForGear.Map.mapPawns.AnyFreeColonistSpawned) 
					&& (!selPawnForGear.IsPrisonerOfColony || (!PrisonBreakUtility.IsPrisonBreaking(selPawnForGear) 
		...
		private Pawn SelPawnForGear
		{
			get
			{
                ...
                string text = "Gear tab on non-pawn non-corpse ";
                Thing selThing = base.SelThing;
                throw new InvalidOperationException(text + ((selThing != null) ? selThing.ToString() : null));
            }
		}
		...
		protected override void FillTab()
		{
			...
			if (!viewList)
			{
				...
				{
					Rect rectstat = new Rect(530f, 0f, 138f, 80f);
			...
			if (this.ShouldShowOverallArmor(this.SelPawnForGear))
			{
				...
				{
                    Rect rectarmor = new Rect(530f, 84f, 138f, 120f);
                    ...
                    if (RPG_ModCheck.IsRWoMActive())
                    {
                        ...
                        TryDrawOverallArmor_RPG(rectharmony, RPG_ModCheck.GetHarmonyStatDef(), "RWoM_ArmorHarmony".Translate(),
		...
		private void DrawThingRow_RPG(Rect rect, Thing thing, bool inventory = false)
		{
			...
			if (Mouse.IsOver(rect))
			{
				...
				if (this.CanControl && (inventory || this.CanControlColonist || (this.SelPawnForGear.Spawned && !this.SelPawnForGear.Map.IsPlayerHome)))
				{
					...
					bool flag4 = apparel != null && this.SelPawnForGear.apparel != null && this.SelPawnForGear.apparel.IsLocked(apparel);
					...
					if (Widgets.ButtonImage(rect2, TexButton.Drop, color, mouseoverColor, !flag, null) && !flag)
					{
            ...
            Text.WordWrap = true;
			TooltipHandler.TipRegion(rect, thing.GetTooltip().text + "\n" + smass);
		}
		...
		public void TryDrawOverallArmor_RPG(Rect rect, StatDef stat, string label, Texture image)
		{
			...
			Rect rect2 = new Rect(rect.x + 60f, rect.y + 3f, 104f, 22f);
		...
		private void TryDrawMassInfo_RPG(Rect rect)
		{
            ...
            Rect rect1 = new Rect(rect.x, rect.y + 2f, 24f, 24f);
            ...
            Rect rect2 = new Rect(rect.x + 40f, rect.y + 3f, 104f, 22f);
		...
		private void TryDrawComfyTemperatureRange_RPG(Rect rect)
		{
			...
			Rect rect1 = new Rect(rect.x, rect.y + 28f, 24f, 24f);
			...
			Rect rect2 = new Rect(rect.x + 60f, rect.y + 29f, 104f, 22f);
            Widgets.Label(rect2, statValue.ToStringTemperature("F0"));

            Rect rect3 = new Rect(rect.x, rect.y + 54f, 24f, 24f);
			GUI.DrawTexture(rect3, ContentFinder<Texture2D>.Get("UI/Icons/Max_Temperature", true));
			TooltipHandler.TipRegion(rect3, "ComfyTemperatureRange".Translate());
			float statValue2 = this.SelPawnForGear.GetStatValue(StatDefOf.ComfyTemperatureMax, true, -1);
			Rect rect4 = new Rect(rect.x + 60f, rect.y + 55f, 104f, 22f);
            Widgets.Label(rect4, statValue2.ToStringTemperature("F0"));
        }
		...
		{
			return p.RaceProps.Humanlike || this.ShouldShowApparel(p) || p.GetStatValue(StatDefOf.ArmorRating_Sharp, true, -1) > 0f 
				|| p.GetStatValue(StatDefOf.ArmorRating_Blunt, true, -1) > 0f || p.GetStatValue(StatDefOf.ArmorRating_Heat, true, -1) > 0f;
		}
